...
const splitContacts = (contacts: User[]): SplitContacts => {
  return contacts.reduce(
    (acc: SplitContacts, contact: User) => {
      if (contact.isRandom) {
...
export function ContactsList({
  ...
  const handleContactClick = async (contact: User) => {
    ...
    if (contact.isRandom) {
      router.push(
        `/chat/${contact.id}?username=${contact.username}&isRandom=true`
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/friend/chat-room?friendId=${contact.id}`
      );
      const data = await response.json();

      if (data.roomId) {
        router.push(
          `/chat/${data.roomId}?username=${contact.username}&isRandom=false`
        );
      }
    } catch (error) {
            ...
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8 p-0">
                ...
                <span className="sr-only">
                  More options for {contact.username}
                </span>
              </Button>
            </DropdownMenuTrigger>
