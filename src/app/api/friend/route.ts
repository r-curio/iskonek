...
export async function POST(request: Request) {
  ...
  const { data: existingFriendship } = await supabase
    ...
    .select()
    .or(
      `and(from_user_id.eq.${user.id},to_user_id.eq.${friend_id}),and(from_user_id.eq.${friend_id},to_user_id.eq.${user.id})`
    )
    .single();
...
export async function PUT(request: Request) {
  ...
  const { error: newRoomError } = await supabase.from("chat_rooms").insert({
    user1_id: user.id,
    user2_id: id,
    status: "active",
    created_at: new Date().toISOString(),
